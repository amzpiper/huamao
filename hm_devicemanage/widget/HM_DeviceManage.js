var HM_DeviceManage = StudioWidgetWrapper.extend({
    init: function () {
        var thisObj = this;
        ...
        if ((typeof (Studio) != "undefined") && Studio) {
            // 51地图定制动作，开启取点工具
            Studio.registerEvents(thisObj, "openPointTool", "开启取点工具", []);
            Studio.registerEvents(thisObj, "buildingDismantling", "楼宇拆解", []);
            Studio.registerEvents(thisObj, "hidePipeline", "管网操作", []);
            Studio.registerEvents(thisObj, "setCamera", "设置场景镜头", []);
            ...
            Studio.registerAction(thisObj, "showEnvSensorWindow", "查看传感器", [], $.proxy(this.poiClickCbk, this), []);
        }
    },
        ...
        if (elem) {
            ...
            thisObj.vm = new Vue({
                ...
                data: {
                    ...
                    LT_spaceId: "",
                    devicePng: {
                        EnvSensor: widgetBasePath + 'images/EnvSensor.png',
                        EnvSensor_selected: widgetBasePath + 'images/EnvSensor-selected.png',
                        EnvSensor_stop: widgetBasePath + 'images/EnvSensor-red.png',

                        camera: widgetBasePath + 'images/camera_icon_blue.png',
                        camera_selected: widgetBasePath + 'images/camera_icon_related.png',

                        Turnstile: widgetBasePath + 'images/Turnstile.png',
                        Turnstile_selected: widgetBasePath + 'images/Turnstile-selected.png',
                    },
                ...
                methods: {
                    ...
                    SynchronousEquipment() {
                        ...
                        let params = {
                            "params": ["hm_bigScreen__LT_Project", "hm_bigScreen__LT_Token"]
                        };
                        thisObj.callAPIConn("/SmartCampus__FacilityManagement/0.1.0/SystemParams/query", params, 'POST', function (vm) {
                            if (vm && vm.resp && vm.resp.code == '0') {
                                ...
                                _this.LT_Project = data['hm_bigScreen__LT_Project'];
                                _this.LT_Token = data['hm_bigScreen__LT_Token'];
                                //查询空间信息id
                                var spaceRootParams = {
                                    start: 0,
                                    limit: 5000,
                                ...
                                thisObj.callAPIConn('/Space/0.1.0/SpaceRoot/query', spaceRootParams, 'POST', function (vm) {
                                    if (vm && vm.resp && vm.resp.code == '0') {
                                        if (vm.data[0]) {
                                            ...
                                            // console.log("data.spaceRoots:", data.spaceRoots)
                                            // console.log("data.spaceId:", data.spaceRoots[0]['spaceId'])
                                            _this.LT_spaceId = data.spaceRoots[0]['spaceId']
                                        }
                                        //同步
                                        let param = {
                                            'body': {
                                                "token": _this.LT_Token,
                                                "project": _this.LT_Project,
                                                "spaceId": _this.LT_spaceId
                                            }
                                        }
                                        thisObj.callAPIConn('/hm_bigScreen__HMSecurityManagement/0.1.0/Device/SyncDevice_LT', param, 'POST', res => {
                                            // console.log("res", res)
                                            ...
                                            _this.$message({
                                                showClose: true,
                                                message: _this.lang.syncSuccess,
                                                type: 'success',
                    ...
                    // Translation: Change all ElementIU components language.
                    renderHeader(h, {column, $index}, tableTitle) {
                        return h(
                            'el-tooltip', {
                                props: {
                                    content: tableTitle,
                    ...
                    setDevStyle() {
                        if (!Studio.inReader) {
                            ...
                            $('#HM_DeviceManage .deviceManage-area').css('width', '40rem');
                        }
                    },
                    openDeviceFacility: function (row) {
                        ...
                        //     type: 'DYNAMIC'
                        // }); //进入设施画像页面
                        if (row.deviceDef.code === 'camera') {
                            let codes = {
                                cameraCodes: [row.externalCode],
                                cameraName: row.deviceName,
                                displayCloseButton: "true"
                            };
                            _this.startLive(codes);
                        } else if (row.deviceDef.code === 'EnvSensor') {
                            _this.deviceEnvSensorDialog.dialog = true;
                            _this.queryEnvSensor(row.externalCode).then(data => {
                                _this.deviceEnvSensorDialog.envSensor.basic = {
                                    name: data.Name,
                                ...
                                _this.deviceEnvSensorDialog.envSensor.attrs = arr;
                            });
                        } else {
                            _this.$message({
                                showClose: true,
                                message: `闸机 - ${row.deviceName}`,
                                type: 'success',
                                customClass: 'hw-message'
                            });
                        }
                    ...
                    queryDeviceByCondition: function (flag) {
                        ...
                        if (flag === 'space') {
                            thisObj.triggerEvent("addMarker", {append: false});
                            _this.apartFloor();
                            this.bdFloor = 40;
                    ...
                    handleCommand: function (command) { //处理表格行中下拉菜单的操作
                        ...
                        switch (command.type) {
                            ...
                            case 'RELATED': {
                                ...
                                break;
                            case 'COLLECT': {
                                thisObj.triggerEvent('openPointTool', {operation: 'POI'});
                                ...
                                break;
                            case 'STOPC': {
                                thisObj.triggerEvent('openPointTool', {operation: 'close'});
                    ...
                    saveSpace: function () {
                        ...
                        // 拆楼
                        let spaceDetail = _this.space.currentRow;
                        if (spaceDetail.spaceAttribute) {
                            let floor = Number(spaceDetail.spaceAttribute[0].attrValue);
                            _this.apartFloor(floor);
                            this.bdFloor = floor;
                        }
                        ...
                        thisObj.callAPIConn('/Device/0.1.0/DeviceDefCategoryTree', params, 'POST', function (vm) {
                            if (vm && vm.resp && vm.resp.code == '0') {
                                ...
                                if (data.categories) {
                                    ...
                                    treeDataed.forEach(item => {
                                        if (item.code === 'huamao') {
                                            _this.deviceDefOptions = item;
                                        }
                                    })
                    ...
                    apartFloor(floor) {
                        ...
                        thisObj.triggerEvent("buildingDismantling", params);
                    },
                    selectSpaceNode: function (data) { //条件中切换位置信息后触发查询事件
                        let _this = this;
                        let spaceParam = {spaceId: data[data.length - 1]};
                        // 查询选中的空间 -> 组装拆楼入参 -> 触发组件拆楼动作
                        this.querySpace(spaceParam).then(spaces => {
                            let space = spaces[0];
                            let code = space['level']['code'];
                            let apartNum = 40;
                            if (code === 'FLOOR') {
                                let attrs = space['spaceAttribute'];
                                let floor = this.getAttrValue(attrs, '', 'up_floor_num');
                                if (floor) {
                                    apartNum = Number(floor);
                                }
                            }
                            if (apartNum === 40) {
                                thisObj.triggerEvent("addMarker", {append: false});
                            ...
                            if (code !== 'ROOM') {
                                _this.apartFloor(apartNum)
                            }

                            // 切换场景镜头中心楼层
                            let jsonData = {
                                center: getCenter(space.longitude, space.latitude, Number(apartNum))
                            };
                            thisObj.triggerEvent("setCamera", jsonData);

                            function getCenter(x, y, floor) {
                                let lon;
                                let lat;
                                let alt;
                                if (floor < 0) {
                                ...
                                return [lon, lat, alt + 200]
                            }

                            this.bdFloor = apartNum;
                        });
                    ...
                    querySpace(condition) {
                        ...
                        if (condition) {
                            ...
                            if (condition.spaceCode) {
                                param.condition.code = {
                                    "operator": "=",
                                    "value": condition.spaceCode
                                };
                            }
                            if (condition.parentId) {
                                param.condition.parent = {
                                    "operator": "=",
                                    "value": condition.parentId
                                };
                            }
                        }
                        return new Promise((resolve, reject) => {
                            thisObj.callAPIConn('/Space/0.1.0/Space/query', param, 'POST', res => {
                                if (res && res.resp && res.resp.code && res.resp.code === '0') {
                                    let spaces = res.data[0].spaces;
                                    // console.log("querySpace:", spaces);
                                    resolve(spaces);
                                } else {
                                    _this.showDialog('error', res.resMsg);
                                    reject(res.resMsg);
                                }
                            });
                        });
                    },
                    getAttrValue(attrs, id, code) {
                        let value = '';
                        for (let i = 0; i < attrs.length; i++) {
                            if (attrs[i].attrDef.code === code || attrs[i].attrDef.id === id) {
                                value = attrs[i].attrValue;
                            }
                        }
                        return value;
                    },
                    ...
                    queryDeviceDef: function () {
                        ...
                        if (_this.deviceDef.condition.code) {
                            ...
                            params.condition.code = {
                                valueList: ['camera', 'EnvSensor', 'Turnstile'],
                                operator: 'in'
                            }
                        }
                        ...
                        function handelDeviceListData(data) {
                            ...
                            _this.deviceList.dataList.forEach(function (e) {
                                ...
                                var marker = {
                                    ...
                                    height: 40,
                                    coord_z: 0,
                                    image: e.deviceDef['iconUrl'] || _this.devicePng[e.deviceDef.code],
                                    imageSelected: _this.devicePng[`${e.deviceDef.code}_selected`],
                                    //以下入参超图无
                                    label: {
                                        text: eDeviceName
                                    },
                            ...
                            if (_this.notInitQuery) {
                                setTimeout(() => {
                                    if (paramObj.markers.length > 0 && _this.bdFloor !== 40) {
                                        thisObj.triggerEvent("addMarker", {append: false, clusterInfo: paramObj});
                                    } else {
                                        _this.handleDeviceIsNotFloor();
                                        thisObj.triggerEvent("addMarker", {append: false});
                                    }
                                }, 5e2);
                            } else {
                    ...
                    handleDeviceIsNotFloor() {
                        let _this = this;
                        let url = "/SmartCampus__FacilityManagement/0.1.0/Device/queryDevice";
                        let params = {
                            condition: {},
                            start: 0,
                            limit: 5000
                        ...
                        thisObj.callAPIConn(url, params, 'POST', res => {
                            if (res.resp && res.resp.code && res.resp.code === '0') {
                                let arr = res.data[0].devices;
                                let paramObj1 = {
                                    layerId: "layer_123456",
                                    layerIndex: 1234,
                                    markers: [],
                                ...
                                arr.forEach(item => {
                                    if (item.deviceLocation && item.deviceLocation.spaceInPath && item.deviceLocation.spaceInPath[0]) {
                                        let spaceLevel = item.deviceLocation.spaceInPath[0]['spaceLevelCode'];
                                        if (spaceLevel !== 'FLOOR') {
                                            let marker = {
                                                id: item.id,
                                                ...
                                                height: 40,
                                                coord_z: 0,
                                                image: item.deviceDef['iconUrl'] || _this.devicePng[item.deviceDef.code],
                                                imageSelected: _this.devicePng[`${item.deviceDef.code}_selected`],
                                                label: {
                                                    text: item.deviceName
                                                }
                                            };
                                            if (item.deviceLocation) {
                                                if (item.deviceLocation.coordinate && (item.deviceLocation.coordinate.indexOf(',') !== -1)) {
                                                    let coordinates = [];
                                                    coordinates = item.deviceLocation.coordinate.split(',');
                                                    if (!coordinates[0] || !coordinates[1]) {
                                                        if (item.deviceLocation['spaceData']) {
                                                            if (item.deviceLocation['spaceData']['longitude'] && item.deviceLocation['spaceData']['latitude']) {
                                                                let lon = item.deviceLocation['spaceData']['longitude'];
                                                                let lat = item.deviceLocation['spaceData']['latitude'];
                                                                marker.position = [lon, lat, 0];
                                                            }
                                                        }
                                                    } else {
                                                        if (coordinates.length < 3) {
                                            ...
                                            paramObj1.markers.push(marker);
                                        }
                                    }
                                });
                                if (_this.APIAlready === '0') {
                                    thisObj.triggerEvent("addMarker", {append: false, clusterInfo: paramObj1});
                                } else {
                    ...
                    querySysParams: function () {
                        ...
                        thisObj.callAPIConn("/SmartCampus__FacilityManagement/0.1.0/SystemParams/query", params, 'POST', function (vm) {
                            if (vm && vm.resp && vm.resp.code == '0') {
                                ...
                                } else _this.dynamicAttrShowFlag = _this.dynamicAttrShow != "1";
                            }
                        })
                    },
                    ...
                    poiClick(poiId) {
                        let _this = this;
                        this.choosePoi = '';
                        queryDevice({id: poiId}).then(data => {
                            ...
                            if (device.deviceDef.code === 'camera') {
                                ...
                                _this.deviceEnvSensorDialog.dialog = true;
                                _this.queryEnvSensor(device.externalCode).then(data => {
                                    _this.deviceEnvSensorDialog.envSensor.basic = {
                                        name: data.Name,
                                        flashtime: data.Flashtime,
                                    };
                        ...
                        function queryDevice(condition) {
                            let _this = this;
                            //根据摄像头id查询摄像头
                            let param = {
                                start: 0,
                            ...
                            if (condition && condition.id) {
                                param.condition.id = {value: condition.id, operator: '='}
                            }
                            return new Promise(resolve => {
                                thisObj.callAPIConn("/Device/0.1.0/Device/query", param, 'POST', (res) => {
                                    if (res && res.resp && res.resp.code == '0' && res.data[0]) {
                                        // console.log("queryDevice", res.data[0]);
                                        resolve(res.data[0])
                                    } else {
                    ...
                    queryEnvSensor(sn) {
                        ...
                        return new Promise(resolve => {
                            thisObj.callAPIConn("hm_bigScreen__HMSecurityManagement/1.0.0/queryEnSensor", param, 'POST', (res) => {
                                if (res && res.resp && res.resp.code == '0' && res.data[0]) {
                                    // console.log("queryEnvSensor", res);
                    ...
                    initMap() {
                        ...
                        let intervalID = setInterval(function () {
                            if (_this.APIAlready === '0') {
                                window.clearInterval(intervalID);
                                //session中获取空间根 ->
                                let currentSpaceData = sessionStorage.getItem("currentSpaceData");
                                if (currentSpaceData) {
                                    let obj = JSON.parse(currentSpaceData);
                                    let space = obj.spaceInfo[0];
                                    _this.apartFloor(40);
                                    if (space.level.code === 'FLOOR') {
                                        // 查询空间 ->
                                        let spaceParam = {spaceId: space.id};
                                        _this.querySpace(spaceParam).then(spaces => {
                                            // 空间地上楼层数 ->
                                            let space = spaces[0];
                                            let attrs = space.spaceAttribute;
                                            let floor = _this.getAttrValue(attrs, '', 'up_floor_num');
                                            if (floor) {
                                                _this.apartFloor(floor);
                                                _this.bdFloor = Number(floor);
                                                // 查询设备,初始化地图点位
                                                _this.condition.spaceInPathLabel = "xxx";
                                                _this.condition.spaceInPath = space.id;
                                                _this.queryDeviceByCondition();
                                            } else {
                                                _this.$message({
                                                    showClose: true,
                                                    message: '当前空间未设置“地上楼层”',
                                                    type: 'warning',
                                                    customClass: 'hw-message'
                                                });
                                            }
    ...
    // 查看传感器动作回调
    poiClickCbk(data) {
        if (!this.vm.choosePoi || this.vm.choosePoi !== data.eventParam.poiId) {
            this.vm.poiClick(data.eventParam.poiId);
            this.vm.choosePoi = data.eventParam.poiId;
        }
    },
