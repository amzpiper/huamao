var huamao_login_widget = StudioWidgetWrapper.extend({
    /**
    ...
    */
        ...
        */
        thisObj.vm = new Vue({
            ...
            */
            created() {
                let _this = this;
				console.log("=========================huamaologin created");
				// debugger;
				/*
            ...
            mounted: function () {
				...
				*/
				console.log("=========================huamaologin mounted");
				// debugger;
				this.initLogin();
                // debugger;
            },
            methods: {
                initLogin(){
                    ...
                    thisObj.callFlowConn("/Common/0.1.0/getSysParameters", param,"POST", function (vm) {
                        if (vm && vm.resp && vm.resp.code == '0') {
                            if (vm.data[0]) {
                                let data = vm.data[0].parameters;
                                potalIndexUrl = data.hmPortalIndexUrl;
                                console.log("data.hmPortalIndexUrl",data.hmPortalIndexUrl)
                            }
                        }
                    })

                    //https://localhost:8001/preview?userToken=4047e85256db44489c5989b8038548fb&Uid=209662984631353363
                    //https://studio.e.huawei.com/magno/render/hm_bigScreen__HMSystemManagement_0000000000hdy26HBtdx/HuamaoLogin?userToken=4047e85256db44489c5989b8038548fb&Uid=209662984631353363
                    //https://studio.e.huawei.com/magno/render/hm_bigScreen__HMSystemManagement_0000000000hdy26HBtdx/HuamaoLogin?userToken=a868c0b552db423c919fbd4eaebc0e09&Uid=209662984631353363
                    //https://abc.hicampuscube.com/magno/render/hm_bigScreen__HMSystemManagement_0000000000hdy26HBtdx/HuamaoLogin?userToken=a868c0b552db423c919fbd4eaebc0e09&Uid=209662984631353363
                    //https://abc.hicampuscube.com/magno/render/SmartCampus__SystemManagement_0000000000isilxEPk7V/HuamaoLogin?userToken=a868c0b552db423c919fbd4eaebc0e09&Uid=209662984631353363
                    //调用IOC第三方登录接口
                    _this.location = window.location.href;
                    console.log("location:",_this.location)
                    str = _this.location
                    var n = str.indexOf("userToken=");
                    var text = str.substr(n,str.length)
                    console.log("text:"+text)
                
                    var nParamOne = text.indexOf("userToken=");
                    var nParamTwo = text.indexOf("&");
                    var paramOne = text.substr(nParamOne+10,nParamTwo-10)
                    console.log("nParamOne:"+nParamOne)
                    console.log("nParamTwo:"+nParamTwo)
                    console.log("paramOne:"+paramOne)
                    var paramTwo = text.substr(nParamTwo+5,str.length)
                    ...
                    thisObj.callFlowConn("/hm_bigScreen__HMSystemManagement/1.0.0/oauthLogin", param, "POST",function (vm) {
                        ...
                        if (vm && vm.resp && vm.resp.code == '0') {
                            ...
                            var message = '';
                            console.log("错误代码:",vm.resCode)
                            if (vm.resCode == 'SmartCampus__LoginValidateError') {
                                message = '(原因:'+vm.resMsg+')';
                            }
                            
                            console.log("第三方登录失败:")
                            var isR = confirm("登录失败"+message+",是否跳转IOC登录?")
                            if(isR){
                                window.location.href = potalIndexUrl;
                            }else{
                ...
                initLoginOld(){
                    ...
                    let param1 = {
                        "parameterNames": [
                            "IAM_RedirectUrl","PortalIndexUrl","IAM_ClientId","IAM_AuthorizeUrl"
                        ]
                    };
                    // debugger;
                    thisObj.callFlowConn("/Common/0.1.0/getSysParameters", param1,"POST", function (vm) {
                        if (vm && vm.resp && vm.resp.code == '0') {
                            if (vm.data[0]) {
                                ...
                                iamRedirectUrl = "http://www.amzpiper.com/login/authorize?"+
                                ...
                                // iamRedirectUrl = data.IAM_AuthorizeUrl + "?redirect_uri=" + data.IAM_RedirectUrl + "&client_id=" + data.IAM_ClientId + "&state=123&response_type=code"
                                console.log("iamRedirectUrl",iamRedirectUrl)
                                console.log("data.IAM_AuthorizeUrl",data.IAM_AuthorizeUrl)
                                console.log("data.IAM_RedirectUrl",data.IAM_RedirectUrl)
                                console.log("data.IAM_ClientId",data.IAM_ClientId)
                                potalIndexUrl = data.PortalIndexUrl;
                                console.log("data.PortalIndexUrl",data.PortalIndexUrl)
                                //获取当前url地址中的参数列表、参数个数
                                ...
                                //console.log(reqParam.state);
                                console.log(reqParam.code);
                                // 当code=空时,!code=true,进入跳转到第三方获取code参数,第三方获取到code重新返回此页面
                                if(!reqParam.code){
                                    // debugger;
                                    console.log("没有code，请求获取code")
                                    window.location.href = iamRedirectUrl;
                                }else{
                                    // host + /service/hqwangl__SecutityManagement/1.0.1/authCallback
                                    console.log("有code，请求登录IOC")
                                    let param = {code:reqParam.code}
                                    // debugger;
                                    // /service/hm_bigScreen__HMSystemManagement/1.0.0/oauthLogin
                                    // ioc的url:https://studio.e.huawei.com/magno/render/SmartCampus__UnifiedPortal_0000000000hdy26HBtdx/MainPortal#c010000000PklZRQv5HM
                                    // https://localhost:8001/preview/

                                    // 测试flow：
                                    // 问题:您没有权限执行该脚本，实际上已经登录上IOC
                                    // 测试高级页面:https://localhost:8001/service/hm_bigScreen__HMSystemManagement/1.0.0/oauthLogin
                                    // 403 {"resCode":"405230606","resMsg":"您没有足够权限'执行脚本'"}
                                    // 或响应504

                                    // 测试高级页面
                                    // 请求登录接口返回成功，当跳转到ioc还显示的是登录界面
                                    ...
                                    thisObj.callFlowConn("/hm_bigScreen__HMSystemManagement/1.0.0/oauthLogin", param, "POST",function (vm) {
                                        ...
                                        if (vm && vm.resp && vm.resp.code == '0') {
                                            if (vm.data[0]) {
                                                let data = vm.data[0];
                                                console.log(data)
                                                // //window.location.href="https://studio.e.huawei.com/magno/render/SmartCampus__UnifiedPortal_0000000000aIilKxKFA9/MainPortal"
                                                // window.location.href="https://studio.e.huawei.com/magno/render/hqwangl__Portal_0000000000aIilKxKFA9/WHQMainPortal"
                                                // debugger;
                                                console.log("window.location.href=",potalIndexUrl)
                                                // window.location.href = potalIndexUrl;

                                                //test
                                                window.location.href = "https://studio.e.huawei.com/magno/render/SmartCampus__UnifiedPortal_0000000000hdy26HBtdx/MainPortal#c010000000PklZRQv5HM"
                                            }
                                        }
                                    })
                                }
	...
	callFlowConn2: function (service, param, type, callbackFunc, failCallBackFunc) {
		var thisObj = this;
		var connector = null;
		switch (type.toUpperCase()) {
			case 'POST': connector = thisObj.getConnectorInstanceByName('PostAPIConnector'); break;
			case 'GET': connector = thisObj.getConnectorInstanceByName('GetAPIConnector');
				if (connector) {
			...
			default: connector = thisObj.getConnectorInstanceByName('PostAPIConnector'); break;
		}
		if (connector) {
			...
			connector
				...
				.fail(function (response) {
					...
					thisObj.vm.$alert(response.response.resMsg, '错误', {
    ...
    callFlowConn: function (service, param, type, callbackFunc) {
        var thisObj = this;
        var connector = null;
        switch (type.toUpperCase()) {
            case 'POST': connector = thisObj.getConnectorInstanceByName('FlowConnector_POST'); break;
            case 'GET': connector = thisObj.getConnectorInstanceByName('FlowConnector_GET'); break;
            case 'PUT': connector = thisObj.getConnectorInstanceByName('FlowConnector_PUT'); break;
            case 'DELETE': connector = thisObj.getConnectorInstanceByName('FlowConnector_DELETE'); break;
            default: connector = thisObj.getConnectorInstanceByName('FlowConnector_POST'); break;
        }
        if (connector) {
            connector.setInputParams({ service: service, needSchema: 'data' });	//异步（默认）
            if (param.async === false) {
            ...
            connector
                ...
                .fail(function (response) {
                    ...
                    callbackFunc.call(thisObj, response.response);
                })
        } else {
            // console.log("No Flow Connector!");
            //组件开发态，未配置连接器时，返回mockdata
