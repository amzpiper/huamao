<div id="DeviceManage" v-cloak>
	<div class="switch-buttonList manage-left" @click="isShowDeviceManage=!isShowDeviceManage" v-show="isShowDeviceManage">
                ...
                <el-form :model="condition" class="forms">
                    ...
                    <el-row :gutter="10" class="deviceConditionSelect">
                        ...
                        <el-col :span="6">
                            ...
                            <div  class="hw-cascader cascader-one cascader_multiple" @mouseenter="cascaderTitle('cascader-one')">
                                <el-cascader :options="deviceDefOptions"  :props="{ multiple: true }" :placeholder="lang.deviceManage.conditionArea.defIdLabel"
                                             v-model="condition.defIdLabelList" @change="selectdefIdLabelNode"
                                             @expand-change="cascaderNodeAddTitle" clearable collapse-tags filterable></el-cascader>
                            </div>
                        </el-col>
                        <el-col :span="6">
                                ...
                                <el-dropdown trigger="hover" @command="handleCommand"
                                    ...
                                    <el-dropdown-menu slot="dropdown">
                                        ...
                                        <el-dropdown-item :command="{type: 'AttributeDef', row: scope.row}"
                                            icon="el-icon-view">
                                            {{lang.deviceAttributeDialog.dialogTitle}}</el-dropdown-item>
                                        <!-- 显示物模型绑点信息-->
                                        <el-dropdown-item :command="{type: 'DeviceModelCnet', row: scope.row}"
                                            icon="el-icon-connection"
                                            v-show="channelIOT.indexOf(scope.row.channelCode)!=-1" divided>
                                            {{lang.deviceManage.tableArea.deviceModelCnet}}</el-dropdown-item>
                                        <el-dropdown-item :command="{type: 'ACTIVE', row: scope.row}"
                                            ...
                                            divided>
                                            {{lang.deviceManage.tableArea.enable}}</el-dropdown-item>
                                        <el-dropdown-item :command="{type: 'INACTIVE', row: scope.row}"
                                            ...
                                            {{lang.deviceManage.tableArea.disable}}</el-dropdown-item>
                                        <el-dropdown-item :command="{type: 'COLLECT', row: scope.row}"
                                            ...
                                            divided>
                                            {{lang.deviceManage.tableArea.collectionLocation}}</el-dropdown-item>
                                        <el-dropdown-item :command="{type: 'STOPC', row: scope.row}"
                                            icon="el-icon-close"
                                            :disabled="scope.row.status=='DELETED' || scope.row.status=='UNREG' || !scope.row.isCollecting">
                                            {{lang.deviceManage.tableArea.stopAcquisition}}</el-dropdown-item>
                                        <el-dropdown-item :command="{type: 'RELATED', row: scope.row}"
                                            icon="el-icon-connection"
                                            :disabled="scope.row.status=='DELETED' || scope.row.status=='UNREG' || scope.row.isCollecting">
                                            {{lang.deviceManage.tableArea.associatedDevice}}</el-dropdown-item>
                                        <!-- 注册设备到IOT平台（channel在系统参数中这里才可见） -->
                                        <el-dropdown-item :command="{type: 'AddToIOT', row: scope.row}"
                                            icon="el-icon-circle-plus-outline" :disabled="scope.row.status=='DELETED'"
                                            divided v-show="channelIOT.indexOf(scope.row.channelCode)!=-1 && scope.row.status=='ACTIVE'">
                                            {{lang.deviceManage.tableArea.addToIOT}}</el-dropdown-item>
                                        <el-dropdown-item :title="lang.deviceManage.tableArea.onlyUpdateDeviceName" :command="{type: 'UpdateToIOT', row: scope.row}"
                                            :disabled="scope.row.status=='DELETED'" icon="el-icon-edit-outline"
                                            v-show="channelIOT.indexOf(scope.row.channelCode)!=-1 && scope.row.status=='ACTIVE'">
                                            {{lang.deviceManage.tableArea.updateToIOT}}</el-dropdown-item>
                                        <el-dropdown-item :command="{type: 'DeleteFromIOT', row: scope.row}"
                                            icon="el-icon-delete"
                                            v-show="channelIOT.indexOf(scope.row.channelCode)!=-1 && scope.row.status=='ACTIVE'">
                                            {{lang.deviceManage.tableArea.deleteFromIOT}}</el-dropdown-item>
                                        <!-- 废弃、删除 -->
                                        <el-dropdown-item :command="{type: 'DELETED', row: scope.row}"
                                            icon="el-icon-close" :disabled="scope.row.status=='DELETED'" divided>
                                            {{lang.deviceManage.tableArea.deprecated}}</el-dropdown-item>
                                        <el-dropdown-item :command="{type: 'DEL', row: scope.row}"
                                            :disabled="scope.row.status!='DELETED'" icon="el-icon-delete">
                                            {{lang.deviceManage.tableArea.delete}}</el-dropdown-item>
                                    </el-dropdown-menu>
                                </el-dropdown>
                            </div>
                        </template>
                        ...
                        <el-form-item :label="lang.addEditDialog.position">
                            <div v-if="!currentSpace.id" class="hw-input hw-input__small hw-input--border">
                                <el-input v-model="deviceLocation.spaceLabel" readonly class="input-with-search"
                                    ...
                                    @focus="openSpaceDialog" :placeholder="lang.addEditDialog.positionPlaceholder">
                        ...
                        <el-form-item :label="lang.addEditDialog.configurationName">
                            <div class="hw-select hw-select--border">
                                <el-select v-model="deviceData.hmi" filterable
                                           :placeholder="lang.addEditDialog.configurationNamePlaceholder" @change="selectOne">
                            ...
                            <el-form-item :label="lang.addEditDialog.longitude">
                                <div class="hw-input hw-input__small hw-input--border">
                                    <el-input oninput="value=value.replace(/[^\d.-]/g,'')"
                                        v-model="deviceLocation.longitude" :title="deviceLocation.longitude"></el-input>
                                </div>
                            </el-form-item>
                        </el-col>
                        <el-col :span="6">
                            <el-form-item :label="lang.addEditDialog.latitude">
                                <div class="hw-input hw-input__small hw-input--border">
                                    <el-input oninput="value=value.replace(/[^\d.-]/g,'')"
                                        v-model="deviceLocation.latitude" :title="deviceLocation.latitude"></el-input>
                                </div>
                            </el-form-item>
                        </el-col>
                        <el-col :span="4">
                            <el-form-item :label="lang.addEditDialog.height">
                                <div class="hw-input hw-input__small hw-input--border">
                                    <el-input oninput="value=value.replace(/[^\d.-]/g,'')"
                                        v-model="deviceLocation.altitude" :title="deviceLocation.altitude"></el-input>
                                </div>
                            </el-form-item>
                        </el-col>
                        <el-col :span="4">
                                ...
                                <div class="attr-display">
                                    <el-row :gutter="5" v-if="item.valueType!='1'"
                                        ...
                                        <div class="flex">
                                            ...
                                            <el-col :span="6">
                                                <div class="hw-input hw-input__small hw-input--border unitCode" :class="dynamicAttrShowFlag? 'unitCodetrue' : 'unitCodefalse'">
                                                    <el-input v-model="item.attributeValue" :title="item.attributeValue ? (item.unit ? item.attributeValue +' '+item.unit.code : '') :''"
                                                        @blur="item.attributeValue=(item.attributeValue?item.attributeValue.replace(/^\s+|\s+$/g,''):'')" :disabled="dynamicAttrShowFlag">
                                                    </el-input>
                            ...
                            <el-form :model="deviceRelation" :inline="true" class="forms">
                                <el-form-item :label="lang.formitem.associateddevice">
                                    <el-input id="selectOption" readonly="true"
                                        ...
                                        v-model="deviceRelation.selectedData" @click.native="associatedDeviceOpen">
    ...
    <el-dialog id="device-def-dialog" :title="lang.defDialog.dialogTitle" :modal-append-to-body=true
        ...
        :close-on-click-modal=false :visible.sync="deviceDef.dialog" @close="closeDeviceDefDialog" class="hw-dialog"
        width="50rem">
        <el-form :model="deviceDef.condition" :inline="true" class="forms" @submit.native.prevent>
            <!-- <el-form-item>
					<div class="hw-select hw-input__small hw-select--border">
						<el-select v-model="deviceDef.condition.deviceDefCategory" :placeholder="lang.defDialog.specCatalog"
							clearable>
							<el-option v-for="(item, index) in deviceDef.categories" :label="item.categoryName || item.code"
        ...
        <div class="pagination hw-pagination">
            <el-pagination :pager-count="pageCount" @current-change="handleDefCurrentChange"
                :current-page="deviceDef.pageNum" :page-size="deviceDef.pageSize" layout="total, prev, pager, next"
        ...
        <div>
                    ...
                    <div class="hw-select hw-input__small hw-select--border">
                        <el-select v-model="space.condition.spaceRootCode" :placeholder="lang.spaceDialog.spaceRoot"
                            ...
                            <el-option v-for="(item, index) in space.spaceRoots"
                                :label="item.spaceRootName || item.code" :value="item.code" :key="index"></el-option>
                        </el-select>
                    </div>
                </el-form-item>
                <el-form-item>
                    <div class="hw-input hw-input__small hw-input--border">
                        <el-input :placeholder="lang.spaceDialog.spaceName" v-model="space.condition.spaceName"
                            clearable @keyup.enter.native="querySpaceListData" @clear="querySpaceListData"></el-input>
                    </div>
                </el-form-item>
            ...
            <div class="pagination hw-pagination">
                <el-pagination :pager-count="pageCount" @current-change="handleSpaceCurrentChange"
                    :current-page="space.pageNum" :page-size="space.pageSize" layout="total, prev, pager, next"
                    :total="space.total">
                </el-pagination>
            </div>
            <el-divider></el-divider>
        ...
        <div class='distanceSelect-area' v-show="!isShowCameraRange">
            <ul style="list-style:none;padding-inline-start: 0;margin-bottom: 0rem;padding:0;width:100%;">
                <li style="margin: 0 0.5rem 0.25rem 1rem;color: #ffed2a;cursor: none;">
                    {{lang.distanceSelect.cameraRange}}</li>
                ...
                <li v-for="(item, index) in selectCameras.rangedata" :key="index" @click="searchCameras(index)"
                    :class="{rangeActive: selectCameras.classMode[index]}">{{item}}{{lang.distanceSelect.minne}}</li>
            </ul>
        </div>
    </div>
    ...
    <el-dialog id="device-deviceModel-dialog"
        ...
        @mouseenter.native="StyleTitle()" :modal-append-to-body=true :append-to-body=true v-if="deviceModel.dialog"
